...
for(const name of ['index.html','manifest.json','service-worker.js','js','css','images','icons','fonts']) {
    fs.cpSync(path.join(root,name),path.join(out,name),{recursive:true});
}
const commit=process.env.GITHUB_SHA || execFileSync('git',['rev-parse','HEAD'],{cwd:root,encoding:'utf8'}).trim();
fs.writeFileSync(path.join(out,'build-info.json'),JSON.stringify({commit,saveVersion:5,built:new Date().toISOString()},null,2));
